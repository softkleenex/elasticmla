...
def main():
 ...
 init=torch.load(args.base_dir/"router_max.pt",map_location="cpu",weights_only=False);ck=torch.load(args.checkpoint,map_location=d,weights_only=False);base=MLAGPT(**ck["config"]).to(d).eval();base.load_state_dict(ck["model"]);tiers=args.tiers or oracle["deployment_tiers"]
