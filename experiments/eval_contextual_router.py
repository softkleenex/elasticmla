...
def main():
 d=dev();summary_path=BASE/"contextual_oracle_v1/contextual_oracle_v1_summary.json";records_path=BASE/"contextual_oracle_v1/contextual_oracle_v1_records.json";oracle=json.load(open(summary_path));records=json.load(open(records_path));orders=[torch.tensor(o) for o in oracle["layer_channel_orders"]];tiers=oracle["deployment_tiers"]
 if file_sha(CKPT)!=oracle["checkpoint_sha256"] or file_sha(DATA)!=oracle["data_sha256"]:raise ValueError("evaluation inputs do not match oracle")
 ck=torch.load(CKPT,map_location=d,weights_only=False);base=MLAGPT(**ck["config"]).to(d).eval();base.load_state_dict(ck["model"]);policies={};splits=[]
 for name in ("mean","max"):
  c=torch.load(BASE/f"router_{name}.pt",map_location="cpu",weights_only=False)
 ...
 test=splits[0]["test"];starts={r["seq"]:r["sequence_start"] for r in records};data=np.memmap(DATA,dtype=np.uint16,mode="r");full=[];fixed={t:{"loss":[],"bytes":[]} for t in tiers};acc={n:{"loss":[],"shuffle":[],"bytes":[],"sum":0,"n":0,"hist":{str(t):0 for t in tiers}} for n in policies};N=20
 with torch.no_grad():
  for s in test:
