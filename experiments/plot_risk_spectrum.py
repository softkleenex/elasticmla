...
ax = axes[0]
for name, s in (("30M", s30), ("122M", s122)):
...
for i, (name, s) in enumerate((("30M", s30), ("122M", s122))):
    d = s["mean_safe_rate_positive_part_diagnostics"]
    vals = [d["signed_mean"], d["positive_part_mean"], d["maximum"]]
    ax.bar(x + (i - 0.5) * width, vals, width, color=colors[name], label=name)
...
for i, (name, s) in enumerate((("30M", s30), ("122M", s122))):
    ...
    ax.text(0 + (i - 0.5) * width, s["mean_safe_rate_positive_part_diagnostics"]["signed_mean"] + .01,
            f"{frac*100:.0f}% of\nrecords\nexceed epsilon", fontsize=6.5, ha="center", color=colors[name])
